...
KEYS=["store_id","daypart","channel"]
CATS=["store_id","daypart","channel","region","store_type"]
...
def train_and_predict(source=SALES_PATH):
    df=make_features(pd.read_csv(source)); cutoff=df.date.max()-pd.Timedelta(days=FORECAST_HORIZON_DAYS-1)
